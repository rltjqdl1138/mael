...
export default class SearchContainer extends Component {
    ...
    render(){
        const {handler, searchOpen, headerPos} = this.props
        const {suggestList} = this.state
        const height = Math.min(suggestList.length , MAX_CONTENTS ) * 30 + 94
        return (
            <View style={[styles.container,{display:searchOpen?'flex':'none'}]}>
                <TouchableWithoutFeedback 
                    onPress={()=>{
                        this.state.isTyping? Keyboard.dismiss() : handler(false);
                        this.handleChange('isTyping', false)  }}>
                ...
                <View style={[styles.mainContainer,{top:headerPos-10, height: height}]}>
                    <View style={styles.searchContainer}>
                        ...
                        <TextInput
                            ...
                            numberOfLines={1}
                            onFocus={()=>{this.handleChange('isTyping',true)}}
...
class SuggestItem extends Component{
    render(){
        ...
        const SuggestItem = list.map( item =>{
            return(
                <TouchableOpacity style={styles.suggestContentContainer}>
                    <Image style={styles.searchImage} 
                        source={require('../icon/search.png')}/>
